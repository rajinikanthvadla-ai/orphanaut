...
def create_session(config: AuthConfig) -> boto3.Session:
    ...
    raise AuthenticationError(f"Unsupported auth method: {config.method}")


def validate_credentials(session: boto3.Session) -> tuple[str, str]:
    """Verify credentials by calling STS GetCallerIdentity. Returns account id and ARN."""
    try:
